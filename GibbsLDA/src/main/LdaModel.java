...
public class LdaModel {
	...
	public LdaModel(double d, double e, int iterations, int numTopics, int saveStep, int beginSaveIteration) {
		this.alpha = d;
		this.beta = e;
		this.iterations = iterations;
		this.K = numTopics;
		this.saveStep = saveStep;
		this.beginSaveIters = beginSaveIteration;
	}
	...
	private void updateEstimatedParameters() {
		...
		for(int m = 0; m < M; m++){
			for(int k = 0; k < K; k++){
				theta[m][k] = (nmk[k] + alpha) / (nmkSum[m] + K * alpha);
			}
		}
	}
	...
	private int sampleTopicZ(int m, int n) {
		...
		for(int k = 0; k < K; k++){
			p[k] = (nkt[k][doc[m][n]] + beta) / (nktSum[k] + V * beta) * (nmk[k] + alpha) / (nmkSum[m] + K * alpha);
		}
	...
	public double logPerplexity() {
		...
		for (int m = 0; m < M; m++) {
			double denominator = this.sum(this.theta[m]);
			for (int k = 0; k < K; k++) {
				theta[m][k] = this.theta[m][k] / denominator;
			}
		}
		double perplexity = 0;
		int numWords = 0;
		for (int i = 0; i < doc.length; i++) {
			for (int j = 0; j < doc[i].length; j++) {
				...
				double wordPpx = 0;
				for (int k = 0; k < K; k++) {
					wordPpx += theta[i][k] * this.phi[k][word];
				}
				perplexity -= Math.log(wordPpx);
			}
		}
		return perplexity / numWords;
	}
}
